...
def main(page: ft.Page):
    ...
    shortcuts = {
        "Нефть": "Н", "Газ": "Г", "Цифровой Доллар": "Д", "Золото": "З",
        "Нейросети": "НС", "Био-Чипы": "БЧ", "Квантовые Ядра": "КЯ", "Темная Материя": "ТМ"
    }

    default_state = {
        "money": 10000, "turn": 1, "level": 1, "goal": 250000,
        "is_blackmailed": False, "mafia_active": False, "bots_count": 0, "has_lawyer": False, "loan": 0,
        ...
        "trends": {"Нефть": ["▬", "▬", "▬"], "Газ": ["▬", "▬", "▬"], "Цифровой Доллар": ["▬", "▬", "▬"],
                   "Золото": ["▬", "▬", "▬"]}
    }
    ...
    def next_turn_click(e):
        ...
        if state["is_blackmailed"]:
            ...
            state["money"] -= tax
            log_message(f"⚠️ ШАНТАЖ: Изъято синдикатом {tax}$", "#FF4A4A")
        ...
        # Повышаем общий шанс любого события до 75% для плотности геймплея
    ...
    def generate_event(total_capital, current_day):
        # --- 💥 СУПЕР-КАПСУЛА: ГЛОБАЛЬНЫЕ КАТАСТРОФЫ (Раз в 150 дней) ---
        if (current_day - state["last_global_crisis_day"]) >= 150 and random.random() < 0.12:
            ...
            global_crises = [
                ...
                {"t": "☀️ ГЕОШТОРМ: Вспышка на Солнце выжгла спутники. Нейросети и Био-Чипы -60%! Золото +70%.",
        ...
        # --- КУЛДАУН: ВЫИГРЫШ В ЛОТЕРЕЮ (Раз в 200 дней, Ровно 33% от капитала) ---
        ...
        # --- КУЛДАУН: НАЛОГОВАЯ ПРОВЕРКА (До 100к капитала, раз в 100 дней) ---
        if total_capital < 100000 and (current_day - state["last_tax_day"]) >= 100 and random.random() < 0.25:
            ...
            if state["has_lawyer"]:
                ...
                log_message(f"⚖️ ФНС: Проверка выявила нарушения. Штраф из наличных: -{tax_penalty:,}$", "#FF4A4A")
            return

        # --- КУЛДАУН: НАЕЗД БАНДИТОВ / МАФИИ (От 100к капитала, раз в 100 дней) ---
        if total_capital >= 100000 and (current_day - state["last_mafia_day"]) >= 100 and not state[
        ...
        # --- 🎲 ВЫБОР ТИПА СОБЫТИЯ: 45% МЕЛКИЙ ФОН, 55% КРУПНЫЕ ИВЕНТЫ ---
        if random.random() < 0.45:
            # --- 📡 МЕЛКИЙ ИНФОРМАЦИОННЫЙ ФОН (СЛУХИ И СПЕКУЛЯЦИИ) ---
            if random.random() < 0.50:
                # 30 МЕЛКИХ СЛУХОВ (ПОЛОЖИТЕЛЬНЫЕ, +2%...+8% к цене или мелкие деньги)
                small_good = [
                    ...
                    {"t": "💬 Экономия: Твой ИИ поменял тариф провайдера. Сэкономлено +150$.", "tgt": None, "mult": 1.0,
                     "m": 150},
                    {"t": "💬 Reddit: Реддиторы хвалят отчетность нефтяников. Нефть +4%.", "tgt": "Нефть", "mult": 1.04,
                     "m": 0},
                    {"t": "💬 Мнение эксперта: Зима обещает быть прохладной. Газ +5%.", "tgt": "Газ", "mult": 1.05,
                     "m": 0},
                    {"t": "💬 Стримеры: Стримеры массово донатят в гос-валюте. Цифровой Доллар +3%.",
                     "tgt": "Цифровой Доллар", "mult": 1.03, "m": 0},
                    {"t": "💬 Хедж-фонд: Один из фондов докупил немного Золота на сдачу: +3%.", "tgt": "Золото",
                     "mult": 1.03, "m": 0},
                    {"t": "💬 Анонс: Новая инди-игра использует нейро-озвучку. Нейросети +4%.", "tgt": "Нейросети",
                     "mult": 1.04, "m": 0},
                    {"t": "💬 Мед-блог: Кибер-протезы пальцев стали популярнее. Био-Чипы +5%.", "tgt": "Био-Чипы",
                     "mult": 1.05, "m": 0},
                    {"t": "💬 Техно-блог: Квантовые Ядра тестируют в домашних ПК. Рост +6%.", "tgt": "Квантовые Ядра",
                     "mult": 1.06, "m": 0},
                    {"t": "💬 Теории заговора: Поговаривают, Темная Материя продлевает жизнь. Цена +8%.",
                     "tgt": "Темная Материя", "mult": 1.08, "m": 0},
                    {"t": "💬 Мелкий бонус: Нашел на улице чистый крипто-чип. Сдан скупщикам: +200$.", "tgt": None,
                     "mult": 1.0, "m": 200},
                    {"t": "💬 Объявление: Продал старые логовские базы данных ботов: +400$.", "tgt": None, "mult": 1.0,
                     "m": 400},
                    {"t": "💬 Слухи: Танкер на заправке застрял всего на час. Мелкий дефицит. Нефть +3%.",
                     "tgt": "Нефть", "mult": 1.03, "m": 0},
                    {"t": "💬 Аналитика: Небольшие заводы переходят на СПГ. Газ +4%.", "tgt": "Газ", "mult": 1.04,
                     "m": 0},
                    {"t": "💬 Опрос: Граждане доверяют электронным кошелькам. Цифровой Доллар +2%.",
                     "tgt": "Цифровой Доллар", "mult": 1.02, "m": 0},
                    {"t": "💬 Ломбарды: Зафиксирован мелкий приток инвесторов в слитки. Золото +3%.", "tgt": "Золото",
                     "mult": 1.03, "m": 0},
                    {"t": "💬 Паблики: Бот-стихоплет стал вирусным в сети. Нейросети +4%.", "tgt": "Нейросети",
                     "mult": 1.04, "m": 0},
                    {"t": "💬 Твиттер: Манипулятор средней руки пампит Био-Чипы: +5%.", "tgt": "Био-Чипы", "mult": 1.05,
                     "m": 0},
                    {"t": "💬 Новости: Институт закупил партию Квантовых Ядер для лабы. Цена +4%.",
                     "tgt": "Квантовые Ядра", "mult": 1.04, "m": 0},
                    {"t": "💬 Лаборатории: Небольшой прорыв в очистке Темной Материи. Стоимость +5%.",
                     "tgt": "Темная Материя", "mult": 1.05, "m": 0},
                    {"t": "💬 Кэшбэк: Пришел мелкий возврат за аренду серверов: +250$.", "tgt": None, "mult": 1.0,
                     "m": 250},
                    {"t": "💬 Подарок: Подписчик твоего ИИ-блога закинул небольшой донат: +350$.", "tgt": None,
                     "mult": 1.0, "m": 350}
                ]
                evt = random.choice(small_good)
                state["money"] += evt["m"]
                if evt["tgt"] and evt["tgt"] in state["prices"]: state["prices"][evt["tgt"]] = int(
                    state["prices"][evt["tgt"]] * evt["mult"])
                log_message(evt["t"], "#707580")  # Серый цвет для фона информационного шума
            else:
                # 30 МЕЛКИХ СЛУХОВ (ОТРИЦАТЕЛЬНЫЕ, -2%...-8% к цене или мелкие траты)
                small_bad = [
                    ...
                    {"t": "💬 Штраф: Небольшое нарушение правил парковки дрона: -150$.", "tgt": None, "mult": 1.0,
                     "m": -150},
                    {"t": "💬 Reddit: Реддиторы шортят региональные нефтяные компании. Нефть -4%.", "tgt": "Нефть",
                     "mult": 0.96, "m": 0},
                    {"t": "💬 Прогнозы: Метеорологи обещают теплую неделю. Газ -5%.", "tgt": "Газ", "mult": 0.95,
                     "m": 0},
                    {"t": "💬 Соцсети: Инфлюенсер раскритиковал комиссии. Цифровой Доллар -3%.",
                     "tgt": "Цифровой Доллар", "mult": 0.97, "m": 0},
                    {"t": "💬 Биржи: Ювелирный завод временно сократил закупки. Золото -2%.", "tgt": "Золото",
                     "mult": 0.98, "m": 0},
                    {
                        "t": "💬 Замедление: Нейросеть-генератор картинок временно упала из-за наплыва юзеров. Нейросети -4%.",
                        "tgt": "Нейросети", "mult": 0.96, "m": 0},
                    {"t": "💬 Таблоиды: Кибер-модели жалуются на дискомфорт от чипов. Био-Чипы -5%.", "tgt": "Био-Чипы",
                     "mult": 0.95, "m": 0},
                    {"t": "💬 Слухи: Поставки Квантовых Ядер застряли на региональной таможне. Цена -3%.",
                     "tgt": "Квантовые Ядра", "mult": 0.97, "m": 0},
                    {"t": "💬 Спекуляции: Физик-теоретик назвал Темную Материю мифом. Курс -6%.",
                     "tgt": "Темная Материя", "mult": 0.94, "m": 0},
                    {"t": "💬 Поломка: Перегорел кабель питания охлаждения фермы: -300$.", "tgt": None, "mult": 1.0,
                     "m": -300},
                    {"t": "💬 Расходы: Обновил лицензию на торговый ИИ-скрипт: -250$.", "tgt": None, "mult": 1.0,
                     "m": -250},
                    {"t": "💬 Новости: Один из танкеров ушел на мелкий ремонт. Нефть -3%.", "tgt": "Нефть", "mult": 0.97,
                     "m": 0},
                    {"t": "💬 Аналитика: Спрос на отопление в промзонах упал. Газ -4%.", "tgt": "Газ", "mult": 0.96,
                     "m": 0},
                    {"t": "💬 Опрос: Часть инвесторов сомневается в надежности крипто-узлов. Цифровой Доллар -3%.",
                     "tgt": "Цифровой Доллар", "mult": 0.97, "m": 0},
                    {"t": "💬 Продажи: Инвесторы фиксируют копеечную прибыль. Золото -3%.", "tgt": "Золото",
                     "mult": 0.97, "m": 0},
                    {"t": "💬 Сбои: Фильтр спама в ИИ начал блокировать обычные письма. Нейросети -4%.",
                     "tgt": "Нейросети", "mult": 0.96, "m": 0},
                    {"t": "💬 Твиттер: Хейтеры форсят тег против корпорации био-чипов. Цена -4%.", "tgt": "Био-Чипы",
                     "mult": 0.96, "m": 0},
                    {"t": "💬 Задержки: Фабрика перенесла презентацию нового чипсета ядер. Квантовые Ядра -4%.",
                     "tgt": "Квантовые Ядра", "mult": 0.96, "m": 0},
                    {"t": "💬 Опасения: Сложности с транспортировкой контейнеров с изотопами. Темная Материя -5%.",
                     "tgt": "Темная Материя", "mult": 0.95, "m": 0},
                    {"t": "💬 Быт: Пролил кофе на запасную клавиатуру терминала: -100$.", "tgt": None, "mult": 1.0,
                     "m": -100},
                    {"t": "💬 Подписка: Автоматически списались деньги за хранилище бэкапов: -120$.", "tgt": None,
                     "mult": 1.0, "m": -120}
                ]
        ...
        # --- 📈 КРУПНЫЕ ГЕОПОЛИТИЧЕСКИЕ И ФИНАНСОВЫЕ ИВЕНТЫ (ФРС, Войны, Пандемии) ---
        if random.random() < 0.50:
            # --- 🟢 ПУЛ ИЗ 30 ГЕОПОЛИТИЧЕСКИХ / ПОЛОЖИТЕЛЬНЫХ СОБЫТИЙ (Раз в 40 дней) ---
            if (current_day - state["last_good_event_day"]) >= 40 and random.random() < 0.35:
                state["last_good_event_day"] = current_day
                good_events = [
                    {
                    ...
                    {"t": "🤝 ДИПЛОМАТИЯ: Подписан пакт о ненападении в Персидском заливе. Рынок Нефти: +15%.", "m": 0,
                     "tgt": "Нефть", "mult": 1.15},
                    {"t": "🏦 ИНТЕГРАЦИЯ: ВТО признала Цифровой Доллар единым платежным средством метаверсов: +25%.",
                     "m": 0, "tgt": "Цифровой Доллар", "mult": 1.25},
                    {
                    ...
                    {"t": "🇪🇺 ЕВРОСОЮЗ: Сняты пошлины на импортный сжиженный Газ. Цена Газа выросла на +20%.", "m": 0,
                     "tgt": "Газ", "mult": 1.20},
                    {"t": "🛰 КОСМОС: ООН одобрила коммерческую добычу на Луне. Котировки Золота выросли на +25%.",
                     "m": 0, "tgt": "Золото", "mult": 1.25},
                    {
                    ...
                    {
                        "t": "⚡️ ЭНЕРГОПАКТ: Альянс стран заключил соглашение о квотах на чистую энерги. Газ вырос на +15%.",
                    ...
                    {"t": "🧬 БИОТЕХ: Успешно клонированы первые кибернетические органы. Био-Чипы выросли на +30%.",
                     "m": 0, "tgt": "Био-Чипы", "mult": 1.30},
                    ...
                    {"t": "🎁 Кэшбэк от банка: Твой банк обновил систему лояльности. На счет вернулось +1,500$.",
                     "m": 1500, "tgt": None},
                    {"t": "🔋 Энергоэффективность: Твой ИИ оптимизировал энергопотребление ферм. Бонус +2,000$.",
                     "m": 2000, "tgt": None},
                    ...
                    {"t": "📈 Инсайдерский слив: Знакомый хакер подкинул инсайд. Цены на Газ растут на +20%.", "m": 0,
                     "tgt": "Газ", "mult": 1.2},
                    {"t": "🌟 Вирусный твит: Илон Маск опубликовал мем с золотым слитком. Золото выросло на +15%.",
                     "m": 0, "tgt": "Золото", "mult": 1.15},
            ...
            # --- 🔴 ПУЛ ИЗ 30 ГЕОПОЛИТИЧЕСКИХ / ОТРИЦАТЕЛЬНЫХ СОБЫТИЙ (Раз в 40 дней) ---
            if (current_day - state["last_bad_event_day"]) >= 40 and random.random() < 0.35:
                state["last_bad_event_day"] = current_day
                bad_events = [
                    {
                    ...
                    {"t": "⛔️ САНКЦИИ: ООН ввела жесткое эмбарго на экспорт Темной Материи. Цена упала на -35%.",
                     "m": 0, "tgt": "Темная Материя", "mult": 0.65},
                    {"t": "🔥 ДИВЕРСИЯ: Взорван магистральный трансатлантический газопровод. Газ рухнул на -30%.",
                     "m": 0, "tgt": "Газ", "mult": 0.70},
                    {"t": "⚔️ КИБЕР-ВОЙНА: Хакеры Пентагона атаковали ИИ-серверы Азии. Нейросети просели на -25%.",
                     "m": 0, "tgt": "Нейросети", "mult": 0.75},
                    {"t": "📜 НАЦИОНАЛИЗАЦИЯ: Правительство конфисковало частные золотые рудники. Золото упало на -20%.",
                     "m": 0, "tgt": "Золото", "mult": 0.80},
                    {"t": "📉 КРАХ ФАТФ: Международный регулятор признал Цифровой Доллар временно токсичным: -30%.",
                     "m": 0, "tgt": "Цифровой Доллар", "mult": 0.70},
                    {"t": "🛡 БЛОКАДА: Тайвань полностью прекратил отгрузку Квантовых Ядер. Цена просела на -35%.",
                     "m": 0, "tgt": "Квантовые Ядра", "mult": 0.65},
                    {"t": "🛑 ЗАКОНОПРОЕКТ ФРС: Новое постановление ФРС ограничивает оборот токенов. Убыток -4,500$.",
                     "m": -4500, "tgt": None},
                    {"t": "🌍 ЭКО-МАНИФЕСТ: Конференция по климату ввела огромный налог на добычу Нефть. Цена -20%.",
                     "m": 0, "tgt": "Нефть", "mult": 0.80},
                    {
                    ...
                    {"t": "⚡️ Скачок напряжения: В твоем районе сгорел силовой узел. Замена оборудования: -2,000$.",
                     "m": -2000, "tgt": None},
                    ...
                    {"t": "🦠 Компьютерный червь: Торговые терминалы заражены. Срочная очистка систем: -3,000$.",
                     "m": -3000, "tgt": None},
    ...
    page.add(
        ...
        ft.Container(content=ft.Row([
            make_btn("Взять 10к", borrow_click),
            make_btn("Вернуть 10к", repay_click)
        ], alignment=ft.MainAxisAlignment.CENTER, spacing=15)),

        ft.Row([ft.Text("КИБЕР-ИНФРАСТРУКТУРА", size=12, weight="bold", color="#707580")],
               alignment=ft.MainAxisAlignment.CENTER),
        ft.Container(content=ft.Row([
            make_btn("🤖 Бот (25к)", buy_bot_click),
            make_btn("🛡 Адвокат (50к)", buy_lawyer_click)
        ], alignment=ft.MainAxisAlignment.CENTER, spacing=15)),
